	...
	<div class="essb-tabs">
		<div class="essb-tabs-title" style="padding-top: 15px; padding-bottom: 20px;">
			...
			<div class="essb-title-panel-buttons">
				<?php echo '<a href="'.admin_url ("admin.php?page=essb_addons&check=true").'"  text="' . __ ( 'Check for new addons', 'essb' ) . '" class="essb-btn essb-btn-green" style="margin-right: 5px;"><i class="fa fa-refresh"></i>&nbsp;' . __ ( 'Check for new addons', 'essb' ) . '</a>'; ?>			
			</div>
		</div>
		...
		foreach ( $current_list as $addon_key => $addon_data ) {
			...
			if (! $is_installed) {
				if ($addon_data ['price'] != 'FREE') {
					...
					if (ESSBActivationManager::isActivated()) {
						print '<a class="essb-btn" target="_blank"  href="' . $addon_data ['page'] .'&url='.$site_url .'&code='.ESSBActivationManager::getActivationCode() . '">Download Free</a>';
					}
					else {
