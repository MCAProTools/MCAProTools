...
class newsletters_dates_update extends newsletters_dates {
	...
	function get_changelog() {	
		...
		$request_url = NEWSLETTERS_EXTENSION_URL . 'changelog/63/' . $this -> get_remote_request_params();
	...
	function get_version_info($cache = true) {		
		$raw_response = get_transient('newsletters_dates_update_info');
		...
		if (!$raw_response) {		
			...
			$request_url = NEWSLETTERS_EXTENSION_URL . 'updates/63/' . $this -> get_remote_request_params();
			
			$raw_response = wp_remote_request($request_url, $options);			
			set_transient('newsletters_dates_update_info', $raw_response, 86400);
		}
