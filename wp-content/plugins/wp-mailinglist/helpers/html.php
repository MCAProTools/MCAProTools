...
class wpmlHtmlHelper extends wpMailPlugin {
		...
		if (!empty($hook) && $schedules = wp_get_schedules()) {						
			if ($hookinterval = wp_get_schedule($hook, $args)) {
				if ($hookschedule = wp_next_scheduled($hook, $args)) {				
					return $schedules[$hookinterval]['display'] . '<br/><small>' . __('Next: ', 'wp-mailinglist') . ' <b>' . get_date_from_gmt(date_i18n("Y-m-d H:i:s", $hookschedule, true)) . '</b></small>';
				} else {
	...
	function gen_date($format = "Y-m-d H:i:s", $time = false, $gmt = false) {
		if (empty($format)) {
