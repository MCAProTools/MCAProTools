...
if (!class_exists('wpmlBounce')) {
	class wpmlBounce extends wpMailPlugin {
		...
		function validate($data = array()) {
			...
			if (!empty($data)) {
				if (empty($email)) { $this -> errors['email'] = __('No email was specified.', 'wp-mailinglist'); }
				else {
			...
			if (!empty($data)) {
				...
				if (empty($this -> errors)) {
					...
					if ($bounce = $wpdb -> get_row($bouncequery)) {
						...
						. " SET `count` = '" . ((int) $bounce -> count + 1) . "', `status` = '" . esc_sql($status) . "', `modified` = '" . $modified . "' WHERE `id` = '" . $bounce -> id . "' LIMIT 1";
					} else {
						$query = "INSERT INTO `" . $wpdb -> prefix . $this -> table . "` "
						. " (`id`, `email`, `count`, `status`, `history_id`, `created`, `modified`) "
						. " VALUES ('', '" . $email . "', '1', '" . esc_sql($status) . "', '" . $history_id . "', '" . $created . "', '" . $modified . "')";
					}
