...
class wpmlpaginate extends wpMailPlugin {
	...
	function start_paging($page = null) {
		...
		if (count($records) < $this -> allcount && $this -> allcount > $this -> per_page) {			
			...
			$this -> pagination .= '<a class="next-page' . (($this -> page == $totalpagescount) ? ' disabled" onclick="return false;' : '') . '" href="' . $Html -> retainquery(((!empty($this -> sub)) ? 'page=' . $this -> sub . '&amp;' : false) . $this -> pre . 'page=' . ($this -> page + 1) . $search . $orderby . $order . $this -> after) . '" title="' . __('Next Page', 'wp-mailinglist') . '">&#8250;</a>';
