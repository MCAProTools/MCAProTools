...
class Newsletter_List_Table extends WP_List_Table {
    ...
    private function table_data($per_page = 15, $page_number = 1) {	    
		...
		$sql = "SELECT DISTINCT " . $histories_table . ".* FROM " . $histories_table;
		$sql .= " LEFT JOIN " . $historieslists_table . " ON " . $histories_table . ".id = " . $historieslists_table . ".history_id";		
		$sql .= $this -> add_conditions();
		
		$orderby = (empty($_REQUEST['orderby'])) ? $histories_table . '.modified' : $_REQUEST['orderby'];
				...
				switch ($key) {
					...
					case 'list'					:
						if ($val == "all") {
							...
							$query .= " " . $historieslists_table . ".list_id = '" . $val . "'";
							$didvalue = true;
						}
	...
	function column_subject( $item ) {
		...
		$title .= '<strong>';
		$title .= '<a class="row-title" href="' . admin_url('admin.php?page=' . $wpMail -> sections -> history . '&method=view&id=' . $item['id']) . '">' . __($item['subject']) . '</a>';
			...
			if (!empty($mailinglists) && is_array($mailinglists)) {
				...
				foreach ($mailinglists as $mailinglist_id) {
					if ($mailinglist = $Mailinglist -> get($mailinglist_id, false)) {
						$lists .= sprintf('<a href="' . admin_url('admin.php?page=%s&method=%s&id=%s') . '">%s</a>', $wpMail -> sections -> lists, 'view', $mailinglist -> id, __($mailinglist -> title));
	...
	function column_theme_id($item = array()) {
		...
		if (!empty($item['theme_id'])) {
			...
			if ($theme = $Db -> find(array('id' => $item['theme_id']))) :
				$template .= '<a href="" onclick="jQuery.colorbox({iframe:true, width:\'80%\', height:\'80%\', href:\'' . add_query_arg(array('wpmlmethod' => 'themepreview', 'id' => $theme -> id), home_url()) . '\'}); return false;">' . __($theme -> title) . '</a>';
			endif;
		} else {
    ...
    function column_stats($item = array()) {
		...
		$query = "SELECT SUM(`count`) FROM `" . $wpdb -> prefix . $Bounce -> table . "` WHERE `history_id` = '" . $item['id'] . "'";
		$ebounced = $wpdb -> get_var($query);
		$ebouncedperc = (!empty($etotal)) ? (($ebounced / $etotal) * 100) : 0; 
		
		$query = "SELECT COUNT(DISTINCT `email`) FROM `" . $wpdb -> prefix . $Unsubscribe -> table . "` WHERE `history_id` = '" . $item['id'] . "'";
    ...
    function column_sent($item = array()) {
	    ...
	    <?php elseif ($item['scheduled'] == "Y") : ?>
			...
			<br/><small>(<abbr title="<?php echo $item['senddate']; ?>"><?php echo $Html -> gen_date(false, strtotime($item['senddate'])); ?></abbr>)</small>
		<?php elseif ($item['sent'] <= 0) : ?>
			<span class="newsletters_error"><i class="fa fa-floppy-o fa-fw"></i> <?php _e('Draft', 'wp-mailinglist'); ?></span>
		<?php else : ?>
    ...
    function column_post_id($item = array()) {
	    ...
	    if (!empty($item['post_id'])) {
		    if ($post = get_post($item['post_id'])) {
			    $posttext .= sprintf('<a href="%s">%s</a>', get_edit_post_link($item['post_id']), __($post -> post_title));
		    }
	    } else {
