	...
	<form action="?page=<?php echo $this -> sections -> autoresponderemails; ?>&amp;method=mass" onsubmit="if (!confirm('<?php _e('Are you sure you want to execute this action on the selected autoresponder emails?', 'wp-mailinglist'); ?>')) { return false; }" method="post">
    	<div class="tablenav">
        	<div class="alignleft actions">
				<select name="action">
                    ...
                    <option value="send"><?php _e('Send Selected', 'wp-mailinglist'); ?></option>
				</select>
    	...
    	<table class="widefat">
        	<thead>
            	<tr>
                	<td class="check-column"><input type="checkbox" onclick="jqCheckAll(this, '<?php echo $this -> sections -> autoresponderemails; ?>', 'autoresponderemailslist');" name="checkboxall" value="checkboxall" id="checkboxall" /></td>
                	<?php $colspan++; ?>
                    ...
                    <th class="column-autoresponder_id <?php echo ($orderby == "autoresponder_id") ? 'sorted ' . $order : 'sortable desc'; ?>">
						<a href="<?php echo $Html -> retainquery('orderby=autoresponder_id&order=' . (($orderby == "autoresponder_id") ? $otherorder : "asc")); ?>">
							<span><?php _e('Autoresponder', 'wp-mailinglist'); ?></span>
							<span class="sorting-indicator"></span>
						</a>
					</th>
					<?php $colspan++; ?>
        	...
        	<tbody>
        		<?php if (empty($autoresponderemails)) : ?>
        			<tr class="no-items">
						<td class="colspanchange" colspan="<?php echo $colspan; ?>"><?php _e('No autoresponder emails found', 'wp-mailinglist'); ?></td>
					</tr>
        		<?php else : ?>
						...
						<tr class="<?php echo $class = (empty($class)) ? 'alternate' : ''; ?>">
	                        ...
	                        <td>
	                        	<?php if ($aemail -> status == "sent") : ?>
	                            	<span class="newsletters_success"><i class="fa fa-check"></i> <?php _e('Sent', 'wp-mailinglist'); ?></span>
	                            <?php else : ?>
	                            	<span class="newsletters_error"><i class="fa fa-times"></i> <?php _e('Unsent', 'wp-mailinglist'); ?></span>
	                            <?php endif; ?>
	                        </td>
	                        <td>
        	...
        	<div class="alignleft">
				<?php if (empty($_GET['showall'])) : ?>
					<select class="widefat" style="width:auto;" name="perpage" onchange="change_perpage(this.value);">
						...
						<?php $p = 5; ?>
						<?php while ($p < 200) : ?>
							<option <?php echo (!empty($_COOKIE[$this -> pre . 'autoresponderemailsperpage']) && $_COOKIE[$this -> pre . 'autoresponderemailsperpage'] == $p) ? 'selected="selected"' : ''; ?> value="<?php echo $p; ?>"><?php echo $p; ?> <?php _e('per page', 'wp-mailinglist'); ?></option>
							<?php $p += 5; ?>
						<?php endwhile; ?>
