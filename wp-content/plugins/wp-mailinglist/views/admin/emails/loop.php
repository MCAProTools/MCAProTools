		    ...
		    <div class="tablenav">
				<?php if ($_GET['page'] == $this -> sections -> history) : ?>
			    	<div class="alignleft actions">
			        	...
			        	<a href="<?php echo admin_url('admin.php?page=' . $this -> sections -> history . '&amp;method=emails-mass&amp;action=exportall&amp;emails=all&amp;history_id=' . $history -> id); ?>" class="button"><i class="fa fa-download"></i> <?php _e('Export All', 'wp-mailinglist'); ?></a>
			        </div>
			        <div class="alignleft actions">
			...
			<div class="alignleft">
				<?php if (empty($_GET['showall'])) : ?>
					<select class="widefat" style="width:auto;" name="perpage" onchange="change_perpage(this.value);">
						...
						<?php $s = 5; ?>
						<?php while ($s <= 200) : ?>
							<option <?php echo (isset($_COOKIE[$this -> pre . 'emailsperpage']) && $_COOKIE[$this -> pre . 'emailsperpage'] == $s) ? 'selected="selected"' : ''; ?> value="<?php echo $s; ?>"><?php echo $s; ?> <?php _e('emails', 'wp-mailinglist'); ?></option>
							<?php $s += 5; ?>
						<?php endwhile; ?>
