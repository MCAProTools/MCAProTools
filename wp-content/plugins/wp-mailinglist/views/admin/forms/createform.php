<?php if (empty($ajax)) : ?>
	<div class="wrap newsletters">
		<h1><?php _e('Create Subscribe Form', 'wp-mailinglist'); ?></h1>
		...
		<form onsubmit="newsletters_forms_createform(); return false;" action="<?php echo admin_url('admin-ajax.php?action=newsletters_forms_createform'); ?>" method="post" id="newsletters_forms_createform">
					...
					<div id="title-tabs">
						...
						<?php foreach ($languages as $language) : ?>
							<div id="title-tabs-<?php echo $language; ?>">
								<input type="text" class="widefat" name="Subscribeform[title][<?php echo $language; ?>]" value="" id="Subscribeform_title_<?php echo $language; ?>" />
							</div>
						<?php endforeach; ?>
					</div>
			...
			<p class="submit">
				<button type="submit" value="1" id="newsletters_forms_createform_submit" name="createform" class="button button-primary">
					...
					<?php _e('Create Form', 'wp-mailinglist'); ?></button>
				</button>
			</p>
		</form>
<?php if (empty($ajax)) : ?>
...
jQuery(document).ready(function() {
	...
	<?php if ($this -> language_do()) : ?>
		newsletters_focus('#Subscribeform_title_<?php echo $languages[0]; ?>');
	<?php else : ?>
