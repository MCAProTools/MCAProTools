			...
			<?php if (!empty($forms)) : ?>
				<?php foreach ($forms as $form) : ?>
					...
					<tr class="<?php echo $class; ?>" id="form_row_<?php echo $form -> id; ?>">
						...
						<td>
							<label for="form_check_<?php echo $form -> id; ?>"><abbr title="<?php echo esc_attr(stripslashes($form -> modified)); ?>"><?php echo $Html -> gen_date(null, strtotime($form -> modified)); ?></abbr></label>
						</td>
					</tr>
				<?php endforeach; ?>
			<?php else : ?>
		...
		<div class="alignleft">
			<?php if (empty($_GET['showall'])) : ?>
				<select class="widefat" style="width:auto;" name="perpage" onchange="change_perpage(this.value);">
					...
					<?php $p = 5; ?>
					<?php while ($p < 100) : ?>
						<option <?php echo (!empty($_COOKIE[$this -> pre . 'formsperpage']) && $_COOKIE[$this -> pre . 'formsperpage'] == $p) ? 'selected="selected"' : ''; ?> value="<?php echo $p; ?>"><?php echo $p; ?> <?php _e('per page', 'wp-mailinglist'); ?></option>
						<?php $p += 5; ?>
					<?php endwhile; ?>
