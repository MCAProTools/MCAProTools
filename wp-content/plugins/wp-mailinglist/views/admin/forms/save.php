...
function newsletters_forms_field_add(element, target) {
	...
	var loading = '<div id="newsletters_forms_loading_' + field_id + '" class="newsletters_loading postbox"><h2 class="hndle ui-sortable-handle"><span><i class="fa fa-refresh fa-spin"></i> <?php _e('Loading field...', 'wp-mailinglist'); ?></span></h2></div>';
	...
	<?php if (!empty($manualpostboxes)) : ?>
		<?php foreach ($manualpostboxes as $postbox) : ?>
			...
			<?php if (empty($postbox['mandatory'])) : ?>
				postboxhtml += '<div class="newsletters_delete_handle"><a href="" onclick="if (confirm(\'<?php _e('Are you sure you want to delete this field?', 'wp-mailinglist'); ?>\')) { newsletters_forms_field_delete(\'<?php echo $postbox['field_id']; ?>\'); } return false;"><i class="fa fa-times"></i></a></div>';
			<?php endif; ?>
