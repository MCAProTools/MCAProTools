							...
							<div id="group-title-tabs">
								...
								<?php foreach ($languages as $language) : ?>
									<div id="group-title-tabs-<?php echo $language; ?>">
										<input placeholder="<?php echo esc_attr(stripslashes(__('Enter group title here', 'wp-mailinglist'))); ?>" type="text" class="widefat" name="Group[title][<?php echo $language; ?>]" value="<?php echo esc_attr(stripslashes($this -> language_use($language, $this -> Group() -> data -> title))); ?>" id="Group_title_<?php echo $language; ?>" />
									</div>
								<?php endforeach; ?>
							</div>
