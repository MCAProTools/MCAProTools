...
if ($this -> import_process -> queued_items()) {
	$this -> render_message(19, array('<a href="' . admin_url('admin.php?page=' . $this -> sections -> importexport . '&method=clear') . '" onclick="if (!confirm(\'' . __('Are you sure you want to cancel and stop the import?', 'wp-mailinglist') . '\')) { return false; }" class="button"><i class="fa fa-times"></i> ' . __('Stop Import', 'wp-mailinglist') . '</a>'), true);
}
			...
			<form action="?page=<?php echo $this -> sections -> importexport; ?>&amp;method=import&action=import" id="import-form" method="post" enctype="multipart/form-data">
				...
				<table class="form-table">
					<tbody>
						<tr>
							...
							<td>
								<label><input onclick="jQuery('#csvdiv').show(); jQuery('#macdiv').hide();" <?php echo (!empty($_POST['filetype']) && $_POST['filetype'] == "csv") ? 'checked="checked"' : ''; ?> type="radio" name="filetype" value="csv" /> <?php _e('CSV Spreadsheet', 'wp-mailinglist'); ?></label><br/>
				...
				<div id="csvdiv" style="display:<?php echo (!empty($_POST['filetype']) && $_POST['filetype'] == "csv") ? 'block' : 'none'; ?>;">		
					<table class="form-table">
						<tbody>
									...
									<tr>
										...
										<td>
											<span id="mailinglistscolumn" style="display:<?php echo $fieldemaildisplay = ($_POST['fields']['mailinglists'] == "Y") ? 'block' : 'none'; ?>;">
												<b><?php _e('Column Number:', 'wp-mailinglist'); ?></b> <input type="text" class="widefat" style="width:45px;" name="mailinglistscolumn" value="<?php echo $_POST['mailinglistscolumn']; ?>" />
							...
							<?php /*<tr>
								<th><?php _e('Fields to Import', 'wp-mailinglist'); ?></th>
								<td>
									<p class="howto">
										<?php _e('Tick/check the fields which are available in your CSV file that you are uploading.', 'wp-mailinglist'); ?><br/>
										<?php _e('Fill in only column numbers (no letters), where "1" is the first column in your CSV file.', 'wp-mailinglist'); ?>
									</p>
									...
									<div class="scroll-list" style="max-height:300px;">
										<table class="form-table">
											<tbody>
												...
												<!-- Email Address -->
												<tr>
													<th style="padding:15px 0;"><label><input <?php echo $fieldemailcheck = ($_POST['fields']['email'] == "Y") ? 'checked="checked"' : ''; ?> onclick="jQuery('#emailcolumn').toggle();" type="checkbox" name="fields[email]" value="Y" /> <?php _e('Email Address', 'wp-mailinglist'); ?></label></th>
													<td>
														<span id="emailcolumn" style="display:<?php echo $fieldemaildisplay = ($_POST['fields']['email'] == "Y") ? 'block' : 'none'; ?>;">
															<b><?php _e('Column Number:', 'wp-mailinglist'); ?></b> <input type="text" class="widefat" style="width:45px;" name="emailcolumn" value="<?php echo $_POST['emailcolumn']; ?>" />
														</span>
													</td>
												</tr>
												<!-- IP Address -->
												<tr>
													<th style="padding:15px 0;"><label><input <?php echo $fieldip_addresscheck = ($_POST['fields']['ip_address'] == "Y") ? 'checked="checked"' : ''; ?> onclick="jQuery('#ip_addresscolumn').toggle();" type="checkbox" name="fields[ip_address]" value="Y" /> <?php _e('IP Address', 'wp-mailinglist'); ?></label></th>
													<td>
														<span id="ip_addresscolumn" style="display:<?php echo $fieldip_addressdisplay = ($_POST['fields']['ip_address'] == "Y") ? 'block' : 'none'; ?>;">
															<b><?php _e('Column Number:', 'wp-mailinglist'); ?></b> <input type="text" class="widefat" style="width:45px;" name="ip_addresscolumn" value="<?php echo $_POST['ip_addresscolumn']; ?>" />
														</span>
													</td>
												</tr>
												<!-- Date -->
												<tr>
													<th style="padding:15px 0;"><label><input <?php echo $fieldcreatedcheck = ($_POST['fields']['created'] == "Y") ? 'checked="checked"' : ''; ?> onclick="jQuery('#createdcolumn').toggle();" type="checkbox" name="fields[created]" value="Y" /> <?php _e('Date', 'wp-mailinglist'); ?></label></th>
													<td>
														<span id="createdcolumn" style="display:<?php echo $fieldcreateddisplay = ($_POST['fields']['created'] == "Y") ? 'block' : 'none'; ?>;">
															<b><?php _e('Column Number:', 'wp-mailinglist'); ?></b> <input type="text" class="widefat" style="width:45px;" name="createdcolumn" value="<?php echo $_POST['createdcolumn']; ?>" />
														</span>
													</td>
												</tr>
														...
														<tr>
															...
															<td>
																<span id="<?php echo $field -> slug; ?>div" style="display:<?php echo $display = (!empty($_POST['fields'][$field -> slug]) && $_POST['fields'][$field -> slug] == "Y") ? 'block' : 'none'; ?>;">
																	<b><?php _e('Column Number:', 'wp-mailinglist'); ?></b> <input type="text" class="widefat" style="width:45px;" name="<?php echo $field -> slug; ?>column" value="<?php echo $_POST[$field -> slug . 'column']; ?>" />
																</span>
															</td>
														</tr>
													<?php endforeach; ?>
				...
				<div id="macdiv" style="display:<?php echo (!empty($_POST['filetype']) && $_POST['filetype'] == "mac") ? 'block' : 'none'; ?>;">
					...
					<table class="form-table">
						<tbody>
							<tr>
								<th><label for="macfields_fname"><?php _e('First Name', 'wp-mailinglist'); ?></label></th>
								<td>
									<?php if (!empty($fields)) : ?>
									...
									<?php endif; ?>
									<span class="howto"><?php _e('Choose the custom field for the first name value from the vCard to go into.', 'wp-mailinglist'); ?></span>
								</td>
							</tr>
							<tr>
								<th><label for="macfields_lname"><?php _e('Last Name', 'wp-mailinglist'); ?></label></th>
								<td>
									<?php if (!empty($fields)) : ?>
									...
									<?php endif; ?>
									<span class="howto"><?php _e('Choose the custom field for the last name value from the vCard to go into.', 'wp-mailinglist'); ?></span>
								</td>
							</tr>
							<tr>
								<th><label for="macfields_phone"><?php _e('Phone Number', 'wp-mailinglist'); ?></label></th>
								<td>
									<?php if (!empty($fields)) : ?>
									...
									<?php endif; ?>
									<span class="howto"><?php _e('Choose the custom field for the phone number value from the vCard to go into.', 'wp-mailinglist'); ?></span>
								</td>
							</tr>
						</tbody>
					</table>*/ ?>
