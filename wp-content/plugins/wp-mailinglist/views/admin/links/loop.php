	...
	<form action="?page=<?php echo $this -> sections -> links; ?>&amp;method=mass" method="post" onsubmit="if (!confirm('<?php _e('Are you sure you want to apply this action to the selected links?', 'wp-mailinglist'); ?>')) { return false; }">
		<div class="tablenav">
			...
			<div class="alignleft actions">
				<select name="action">
					...
					<option value="reset"><?php _e('Reset', 'wp-mailinglist'); ?></option>
				</select>
		...
		<table class="widefat">
				...
				<tr>
					...
					<th class="column-link <?php echo ($orderby == "link") ? 'sorted ' . $order : 'sortable desc'; ?>">
						<a href="<?php echo $Html -> retainquery('orderby=link&order=' . (($orderby == "link") ? $otherorder : "asc")); ?>">
							<span><?php _e('Link', 'wp-mailinglist'); ?></span>
							<span class="sorting-indicator"></span>
						</a>
					</th>
					<th><?php _e('Clicks', 'wp-mailinglist'); ?></th>
			...
			<tbody>
				<?php if (empty($links)) : ?>
					<tr class="no-items">
						<td class="colspanchange" colspan="<?php echo $colspan; ?>"><?php _e('No links were found', 'wp-mailinglist'); ?></td>
					</tr>
				<?php else : ?>
					<?php $class = false; ?>
					<?php foreach ($links as $link) : ?>
						<tr class="<?php $class = (empty($class)) ? 'alternate' : false; ?>">
							...
							<td>
								<abbr title="<?php echo $link -> modified; ?>"><?php echo $Html -> gen_date("Y-m-d", strtotime($link -> modified)); ?></abbr>
							</td>
						</tr>
					<?php endforeach; ?>
				<?php endif; ?>
			...
			<div class="alignleft">
				<?php if (empty($_GET['showall'])) : ?>
					<select class="widefat" style="width:auto;" name="perpage" onchange="change_perpage(this.value);">
						...
						<?php $p = 5; ?>
						<?php while ($p < 100) : ?>
							<option <?php echo (!empty($_COOKIE[$this -> pre . 'linksperpage']) && $_COOKIE[$this -> pre . 'linksperpage'] == $p) ? 'selected="selected"' : ''; ?> value="<?php echo $p; ?>"><?php echo $p; ?> <?php _e('per page', 'wp-mailinglist'); ?></option>
							<?php $p += 5; ?>
						<?php endwhile; ?>
