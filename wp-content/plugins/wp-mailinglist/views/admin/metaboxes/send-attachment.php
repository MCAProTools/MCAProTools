...
<table class="form-table">
	<tbody>
		<tr>
			<th><label for="sendattachment"><?php _e('Send Attachment(s)', 'wp-mailinglist'); ?></label></th>
			<td>
				<label><input <?php echo (!empty($_POST['attachments'])) ? 'checked="checked"' : ''; ?> onclick="if (jQuery(this).is(':checked')) { jQuery('#attachmentdivinside').show(); } else { jQuery('#attachmentdivinside').hide(); }" type="checkbox" name="sendattachment" value="1" id="sendattachment" /> <?php _e('Yes, I want to attach files to this email', 'wp-mailinglist'); ?></label>
                <span class="howto"><?php _e('You can attach files to this email for your subscribers to receive.', 'wp-mailinglist'); ?></span>
			</td>
		</tr>
	</tbody>
</table>
...
<div id="attachmentdivinside" style="display:<?php echo (!empty($_POST['attachments'])) ? 'block' : 'none'; ?>;">
    <table class="form-table">
    	<tbody>
            <tr>
            	<th><label for="addattachment"><?php _e('Attachments', 'wp-mailinglist'); ?></label></th>
                <td>
                    ...
                    <h4><a href="" id="addattachment" class="button button-secondary" onclick="add_attachment(); return false;"><i class="fa fa-paperclip"></i> <?php _e('Add an attachment', 'wp-mailinglist'); ?></a></h4>
                </td>
            </tr>
        </tbody>
    </table>
	...
	function add_attachment() {
		...
		atthtml += ' <a class="button button-secondary button-small" href="" onclick="if (confirm(\'<?php _e('Are you sure you want to remove this?', 'wp-mailinglist'); ?>\')) { delete_attachment(' + attachmentcount + '); } return false;"><?php _e('Remove'); ?></a>';
		atthtml += '</div>';
