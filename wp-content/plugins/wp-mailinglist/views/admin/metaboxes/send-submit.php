		...
		<div id="misc-publishing-actions">
			...
			<div class="misc-pub-section sendfromwrapper">
				...
				jQuery(document).ready(function() {
					...
					jQuery('#previewemail_button').click(function() { jQuery('#previewemail_div').show(); });
				...
				<div class="misc-pub-section">
					...
					<span id="sendrecurring">
						...
						<?php if (!empty($_POST['sendrecurring']) && $_POST['sendrecurring'] == "Y") : ?>
							<span id="sendrecurring-value">
								<?php echo sprintf(__('Send every %s %s', 'wp-mailinglist'), $_POST['sendrecurringvalue'], $_POST['sendrecurringinterval'], $_POST['sendrecurringlimit']); ?>
								<?php if (!empty($_POST['sendrecurringlimit'])) : ?><?php echo sprintf(__(', %s times', 'wp-mailinglist'), $_POST['sendrecurringlimit']); ?><?php endif; ?>
							</span>
						<?php else : ?>
					...
					<div id="sendrecurringdiv" class="" style="display:none;">
						...
						<div class="form-field">
							...
							<?php $sendrecurringdate = (empty($_POST['sendrecurringdate'])) ? $Html -> gen_date("Y-m-d H:i:s", current_time('timestamp')) : $_POST['sendrecurringdate']; ?>
							...
							<?php echo $Html -> help(__('Specify a starting date and time for the first recurring instance to run. In the format YYYY-MM-DD HH:MM:SS', 'wp-mailinglist')); ?>
						</div>
						<div class="form-field">
    		...
    		<div class="misc-pub-section curtime misc-pub-section-last">
				...
				<div id="timestampdiv" class="" style="display:none;">
					...
					<?php $senddate = (empty($_POST['senddate']) || strtotime($_POST['senddate']) <= current_time('timestamp')) ? $Html -> gen_date("Y-m-d H:i:s") : $_POST['senddate']; ?>
	...
	<div id="major-publishing-actions">
		<?php if (!empty($_POST['ishistory'])) : ?>
			<div id="delete-action">
				<a href="?page=<?php echo $this -> sections -> history; ?>&amp;method=delete&amp;id=<?php echo $_POST['ishistory']; ?>" onclick="if (!confirm('<?php _e('Are you sure you wish to remove this newsletter from your history?', 'wp-mailinglist'); ?>')) { return false; }" title="<?php _e('Remove this newsletter from your history', 'wp-mailinglist'); ?>" class="submitdelete deletion"><?php _e('Delete Email', 'wp-mailinglist'); ?></a>
