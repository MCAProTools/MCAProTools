...
<table class="widefat" id="latestposts_table">
	<thead>
		<tr>
			<th><?php _e('Subject', 'wp-mailinglist'); ?></th>
			<th><?php _e('Interval', 'wp-mailinglist'); ?></th>
			<th><?php _e('Status', 'wp-mailinglist'); ?></th>
			<th><?php _e('Lists', 'wp-mailinglist'); ?></th>
			<th><?php _e('Posts', 'wp-mailinglist'); ?></th>
			<th><?php _e('Actions', 'wp-mailinglist'); ?></th>
		</tr>
	</thead>
	<tbody>
		<?php if (empty($latestpostssubscriptions)) : ?>
			<tr class="no-items">
				<td class="colspanchange" colspan="6"><?php _e('No latest posts subscriptions', 'wp-mailinglist'); ?></td>
			</tr>
		<?php else : ?>
				...
				<tr class="<?php echo $class = (empty($class)) ? 'alternate' : ''; ?>" id="latestposts_row_<?php echo $latestpostssubscription -> id; ?>">
					<td>
						...
						<div class="row-actions">
							<span class="edit"><a href="<?php echo admin_url('admin.php?page=' . $this -> sections -> settings_tasks . '&amp;method=runschedule&amp;hook=newsletters_latestposts&id=' . $latestpostssubscription -> id); ?>"><?php _e('Run Now', 'wp-mailinglist'); ?></a></span>
							<?php if (!empty($latestpostssubscription -> history_id)) : ?>
								<span class="edit">| <a href="<?php echo admin_url('admin.php?page=' . $this -> sections -> history . '&method=view&id=' . $latestpostssubscription -> history_id); ?>"><?php _e('View Newsletter', 'wp-mailinglist'); ?></a></span>
							<?php endif; ?>
						</div>
					</td>
					<td>
						...
						<?php else : ?>
							<span class="newsletters_error"><?php _e('None', 'wp-mailinglist'); ?></span>
						<?php endif; ?>
					</td>
					<td>
					...
					<td>
						...
						<a href="" id="latestposts_delete_<?php echo $latestpostssubscription -> id; ?>" onclick="if (confirm('<?php _e('Are you sure you want to delete this latest posts subscription?', 'wp-mailinglist'); ?>')) { latestposts_del_row('<?php echo $latestpostssubscription -> id; ?>'); } return false;" class="button delrow"><i class="fa fa-trash"></i></a>	
