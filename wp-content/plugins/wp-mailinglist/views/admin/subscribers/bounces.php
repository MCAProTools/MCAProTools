<div class="wrap newsletters">
	<h2>
		<?php _e('Manage Bounces', 'wp-mailinglist'); ?>
		<a href="<?php echo admin_url('admin.php?page=' . $this -> sections -> subscribers . '&amp;method=bounces'); ?>" class="add-new-h2"><?php _e('Refresh', 'wp-mailinglist'); ?></a>
	</h2>
	...
	<form id="posts-filter" action="<?php echo $_SERVER['REQUEST_URI']; ?>" method="post">
    	<?php if (!empty($bounces)) : ?>
            <ul class="subsubsub">
                <li><?php echo (empty($_GET['showall'])) ? $paginate -> allcount : count($bounces); ?> <?php _e('bounces', 'wp-mailinglist'); ?> |</li>
		...
		<p class="search-box">
			...
			<button value="1" type="submit" class="button">
				<?php _e('Search Bounces', 'wp-mailinglist'); ?>
			</button>
		</p>
	</form>
	<br class="clear" />
	...
	<form id="posts-filter" action="?page=<?php echo $this -> sections -> subscribers; ?>&amp;method=bounces" method="get">
    	...
    	<div class="alignleft actions widefat">
	        ...
	        <input type="submit" name="filter" value="<?php _e('Filter', 'wp-mailinglist'); ?>" class="button button-primary" />
	        
	        <script type="text/javascript">			        	        
		    jQuery(document).ready(function() {
			    jQuery('#historiesautocomplete').select2({
				  placeholder: '<?php _e('Search newsletters', 'wp-mailinglist'); ?>',
				  ajax: {
	...
	<form action="<?php echo admin_url('admin.php?page=' . $this -> sections -> subscribers . '&method=bouncemass'); ?>" method="post" onsubmit="if (!confirm('<?php _e('Are you sure you wish to execute this action on the selected bounces?', 'wp-mailinglist'); ?>')) { return false; }">
		<div class="tablenav">
			<div class="alignleft actions">
				<?php if ($this -> get_option('bouncemethod') == "pop") : ?>
                    <a href="?page=<?php echo $this -> sections -> subscribers; ?>&amp;method=check-bounced" class="button" onclick="if (!confirm('<?php _e('Are you sure you wish to check your POP/IMAP mailbox for bounced emails?', 'wp-mailinglist'); ?>')) { return false; }"><?php _e('Check for Bounces', 'wp-mailinglist'); ?></a>
                <?php endif; ?>
				<select name="action" id="newsletters-bounce-action">
					<option value=""><?php _e('- Bulk Actions -', 'wp-mailinglist'); ?></option>
					<option value="delete"><?php _e('Delete Bounces', 'wp-mailinglist'); ?></option>
		...
		<table class="widefat">
			<thead>
				...
				<th class="column-count <?php echo ($orderby == "count") ? 'sorted ' . $order : 'sortable desc'; ?>">
					<a href="<?php echo $Html -> retainquery('orderby=count&order=' . (($orderby == "count") ? $otherorder : "asc")); ?>">
						<span><?php _e('Count', 'wp-mailinglist'); ?></span>
						<span class="sorting-indicator"></span>
					</a>
				</th>
			...
			<tbody>
						...
						<tr class="<?php echo $class = (empty($class)) ? 'alternate' : ''; ?>">
							...
							<td>
								...
								<div class="row-actions">
									<span class="delete"><a class="submitdelete" href="<?php echo admin_url('admin.php?page=' . $this -> sections -> subscribers . '&method=bouncedelete&id=' . $bounce -> id); ?>" onclick="if (!confirm('<?php _e('Are you sure you want to delete this bounce?', 'wp-mailinglist'); ?>')) { return false; }"><?php _e('Delete Bounce', 'wp-mailinglist'); ?></a></span>
				...
				<?php else : ?>
					<tr class="no-items">
						<td class="colspanchange" colspan="<?php echo $colspan; ?>"><?php _e('No bounces were found', 'wp-mailinglist'); ?></td>
					</tr>
				<?php endif; ?>
			</tbody>
		</table>
			...
			<div class="alignleft">
				<?php if (empty($_GET['showall'])) : ?>
					<select class="widefat" style="width:auto;" name="perpage" onchange="change_perpage(this.value);">
						...
						<?php $s = 5; ?>
						<?php while ($s <= 200) : ?>
							<option <?php echo (isset($_COOKIE[$this -> pre . 'bouncesperpage']) && $_COOKIE[$this -> pre . 'bouncesperpage'] == $s) ? 'selected="selected"' : ''; ?> value="<?php echo $s; ?>"><?php echo $s; ?> <?php _e('bounces', 'wp-mailinglist'); ?></option>
							<?php $s += 5; ?>
						<?php endwhile; ?>
