	...
	<form action="?page=<?php echo $this -> sections -> themes; ?>&amp;method=mass" method="post" onsubmit="if (!confirm('<?php _e('Are you sure you wish to execute this action on the selected templates?', 'wp-mailinglist'); ?>')) { return false; }" id="themesform">
		<div class="tablenav">
		...
		<table class="widefat">
			...
			<tbody>
				<?php if (empty($themes)) : ?>
					<tr class="no-items">
						<td class="colspanchange" colspan="<?php echo $colspan; ?>"><?php _e('No templates were found', 'wp-mailinglist'); ?></td>
					</tr>
				<?php else : ?>
			...
			<div class="alignleft">
				<?php if (empty($_GET['showall'])) : ?>
					<select class="widefat" style="width:auto;" name="perpage" onchange="change_perpage(this.value);">
						...
						<?php $p = 5; ?>
						<?php while ($p < 100) : ?>
							<option <?php echo (!empty($_COOKIE[$this -> pre . 'themesperpage']) && $_COOKIE[$this -> pre . 'themesperpage'] == $p) ? 'selected="selected"' : ''; ?> value="<?php echo $p; ?>"><?php echo $p; ?> <?php _e('per page', 'wp-mailinglist'); ?></option>
							<?php $p += 5; ?>
						<?php endwhile; ?>
