...
if (!class_exists('wpMailPlugin')) {
	class wpMailPlugin extends wpMailCheckinit {
		...
		'Autoresponderemail', 'AutorespondersList', 'FieldsForm', 'SubscriberMeta');
		    ...
		    if (!empty($_POST)) {
			    ...
			    if ($mailinglists = $Mailinglist -> select(true)) {
				    foreach ($mailinglists as $id => $title) {
						...
						$checklist .= '<label><input ' . ((!empty($_POST['Subscriber']['mailinglists']) && in_array($id, $_POST['Subscriber']['mailinglists'])) ? 'checked="checked"' : '') . ' type="checkbox" name="' . $fieldname . '[]" value="' . $id . '" id="checklist' . $id . '" /> ' . __($title) . ' (' . $Db -> count(array('list_id' => $id)) . ' ' . __('subscribers', 'wp-mailinglist') . ')</label><br/>';
				    }
			    }
	    ...
	    function ajax_welcomestats() {
			...
			$todate = (empty($_GET['to'])) ? date_i18n("Y-m-d") : $_GET['to'];
		...
		function ajax_testsettings() {
			...
			if (empty($_GET['init']) && !empty($_POST)) {
				...
				if (empty($errors)) {
					$email = esc_html($_POST['testemail']);
		...
		function ajax_testbouncesettings() {
			...
			$type = (empty($_POST['type'])) ? 'imap' : esc_html($_POST['type']);
			$host = esc_html($_POST['host']);
			$user = esc_html($_POST['user']);
			$pass = esc_html($_POST['pass']);
			$port = esc_html($_POST['port']);
		...
		function ajax_addcontentarea() {
			...
			<div class="postbox">
				<h3 class="hndle"><span><?php echo sprintf(__('Content Area %s', 'wp-mailinglist'), $_POST['contentarea']); ?></span></h3>
				<div class="inside">
					<?php
					...
					if (!empty($_POST)) {
						...
						if (empty($errors)) {
							...
							if ($Subscriber -> save($data, true, false, false, true)) {
								$success = true;
								$successmessage = __('Additional data has been saved.', 'wp-mailinglist');
							...
							<?php if (empty($offsite)) : ?>
								...
								jQuery(document).ready(function() {
									jQuery('#<?php echo $this -> pre; ?>-<?php echo $optinid . $field -> slug; ?>').datepicker({
										changeMonth: true,
										changeYear: true,
										yearRange: "<?php echo esc_js(date_i18n("Y", strtotime("-100 years")) . ':' . date_i18n("Y", strtotime("+100 years"))); ?>",
		...
		function render_error($message, $vars = array()) {
			...
			$this -> render('error-top', array('message' => $message), true, 'admin');
		}
		...
		function render_message($message, $vars = array(), $dismissable = true) {
			...
			$this -> render('message', array('message' => $message, 'dismissable' => $dismissable), true, 'admin');
		}
